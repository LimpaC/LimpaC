...
const sidebarSections: SidebarSection[] = [
  ...
  },
  {
    label: "Calculadoras",
    items: [
      { label: "Cartão Digital", to: "/cartao-digital", icon: CreditCard },
    ],
  },
]
...
function AppShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="relative min-h-[100dvh] overflow-hidden bg-[linear-gradient(180deg,#fff8f7_0%,#fffdfc_38%,#f8fafc_100%)] text-slate-950">
      ...
      <div className="pointer-events-none absolute inset-0 [background-image:linear-gradient(rgba(15,23,42,0.7)_1px,transparent_1px),linear-gradient(90deg,rgba(15,23,42,0.7)_1px,transparent_1px)] [background-size:38px_38px] opacity-[0.035]" />

      <aside
        className="fixed inset-y-0 left-0 z-20 flex min-h-screen shrink-0 flex-col overflow-hidden rounded-none border border-l-0 border-slate-200/70 bg-white/88 p-3 shadow-[0_20px_45px_-28px_rgba(15,23,42,0.3)] backdrop-blur-xl transition-[width] duration-500 ease-[cubic-bezier(0.22,1,0.36,1)]"
      ...
      >
        <div className="flex items-start justify-between gap-3 p-2">
          ...
          <Button
            ...
            aria-label={
              isSidebarExpanded ? "Recolher sidebar" : "Expandir sidebar"
            }
          >
            {isSidebarExpanded ? (
        ...
        <nav ref={navContainerRef} className="relative mt-6 flex-1 space-y-5">
          {sidebarSections.map((section) => (
            <div key={section.label} className="space-y-2">
              <p
                className={cn(
                  "px-3 text-[10px] font-semibold tracking-[0.24em] text-slate-400 uppercase transition-opacity duration-300",
          ...
          <div
            className="pointer-events-none absolute top-0 left-0 w-1 rounded-full bg-rose-500 transition-[transform,height,opacity] duration-500 ease-[cubic-bezier(0.22,1,0.36,1)]"
              ...
              <div
                className={cn(
                  ...
                  isCalcRoute
                    ...
                    : "pointer-events-none translate-y-1 scale-95 opacity-0 blur-sm"
                )}
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end" className="min-w-44">
                    <DropdownMenuItem
                      onSelect={() =>
                        window.dispatchEvent(
                          new CustomEvent("limpac:open-goal")
                        )
                      }
                    >
                      Ajustar meta
                    ...
                    <DropdownMenuItem
                      onSelect={() =>
                        window.dispatchEvent(
                          new CustomEvent("limpac:open-cards")
                        )
                      }
                    >
                      Editar cartões
      ...
      <Dialog
        ...
        onOpenChange={setIsCreateOrganizationDialogOpen}
      >
          ...
          <form onSubmit={handleCreateOrganization}>
            <DialogHeader>
              ...
              <DialogDescription className="text-slate-600">
                Crie uma organização para separar metas, cálculos e histórico no
                dashboard.
              </DialogDescription>
            </DialogHeader>
      ...
      <Dialog open={isHelpDialogOpen} onOpenChange={setIsHelpDialogOpen}>
        <DialogContent className="w-[92vw] max-w-2xl rounded-[28px] border-white/70 bg-white/95 shadow-[0_30px_110px_-70px_rgba(15,23,42,0.55)] backdrop-blur-xl">
          <DialogHeader>
            ...
            <DialogDescription className="text-slate-600">
              Os dados desta tela pertencem somente à organização selecionada no
              topo.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="mt-5 grid gap-3 text-sm leading-6 text-slate-600">
            <div className="rounded-2xl border border-slate-100 bg-slate-50/80 p-4">
              Registre a quantidade inicial de cartões digitais para gerar o
              primeiro cálculo de impacto.
            </div>
            <div className="rounded-2xl border border-slate-100 bg-slate-50/80 p-4">
              Depois do primeiro registro, use “Editar cartões” para salvar a
              nova contagem final. O sistema guarda cada alteração no histórico.
            </div>
            <div className="rounded-2xl border border-slate-100 bg-slate-50/80 p-4">
              A meta controla o progresso exibido no dashboard. O relatório usa
              o estado atual e os registros recentes da organização.
            </div>
          </div>
        </DialogContent>
      </Dialog>
  ...
  return (
    <div className="relative min-h-[100dvh] overflow-hidden bg-[linear-gradient(180deg,#f8fafc_0%,#fffdfc_42%,#fff8f7_100%)] text-slate-950">
      ...
      <div className="px-4 py-6 sm:px-6 lg:px-8">
        <main className="mx-auto max-w-[1500px] min-w-0 pb-6">{children}</main>
      </div>
    </div>
  )
}
