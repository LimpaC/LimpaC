...
export default function Overall() {
  ...
  const maxCards = Math.max(
    1,
    ...(data?.organizations.map((org) => org.latestCalculation?.cards ?? 0) ?? [
      0,
    ])
  )
          ...
          <div className="space-y-3">
            {data?.organizations.map((organization) => {
              ...
              return (
                <div
                  key={organization.id}
                  className="grid gap-2 rounded-2xl border border-slate-100 bg-slate-50/70 p-4 sm:grid-cols-[minmax(0,1fr)_minmax(160px,0.45fr)_auto] sm:items-center"
                >
        ...
        {loading ? (
          ...
          <div className="font-heading text-3xl font-semibold text-slate-950">
            <NumberFlow
              value={value}
              locales="pt-BR"
              format={format ?? { maximumFractionDigits: decimals }}
              suffix={suffix}
            />
          </div>
        )}
